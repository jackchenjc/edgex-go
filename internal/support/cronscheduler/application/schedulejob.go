...
func LoadScheduleJobsToSchedulerManager(ctx context.Context, dic *di.Container) errors.EdgeX {
	...
	for _, job := range jobs {
		...
		if hasMissedAction {
			lc.Debugf("Auto-triggering the missed schedule actions for the scheduled job: %s. Correlation-ID: %s", job.Name, correlationId)
...
func generateMissedRecords(ctx context.Context, job models.ScheduleJob, dic *di.Container) (errors.EdgeX, bool) {
	dbClient := container.DBClientFrom(dic.Get)
	lc := bootstrapContainer.LoggingClientFrom(dic.Get)
	correlationId := correlation.FromContext(ctx)
	...
	if err != nil {
		return errors.NewCommonEdgeX(errors.KindDatabaseError, fmt.Sprintf("failed to load the latest schedule action records of job: %s", job.Name), err), false
	}
